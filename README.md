...
毎日 **06:00 / 12:00 / 18:00（Asia/Tokyo）** に更新する、日本語ニュースダイジェストです。RSS/Atomフィードから候補を集め、過去の掲載履歴と照合して重複を抑え、取得できた見出し・説明・時刻だけを根拠に記事化してGitHub Pagesへ公開します。OpenAIなど外部生成AIのAPIは使用しません。
...
4. 独立した配信元が偏らないよう上限をかけ、通常6〜7本を選定
5. 説明文から「確認できた事実・背景・次に注目する点」を組み立て、各根拠を出典へ結び付ける
...
- 1版につき通常6〜7件を選び、要約だけでなく、確認できた事実、背景、注目点、更新時刻、出典リンクを付けます。
...
- フィードが提供する見出し・短い説明・時刻だけを使うため、元記事だけにある背景や訂正までは反映できません。詳細は必ず出典リンクで確認してください。
